...
lambdaval=[0.001 ];
%variance
sigma_thsqval=[ 1.5 1]; % variance theta
sigma_xsq=1; % variance of source X
rhoval=[0.1 0.5 0.7 0.9 ]; %correlation
% rhoval=[-0.9  ]; 
M=4;
...
save(strcat('threeenc_xtheta_lambda_',num2str(lambda),'M',num2str(M),'rho',num2str(rho),'varth',num2str(sigma_thsq),'varx',num2str(sigma_xsq),'noiseless_xtheta_gaussian.mat'),'xthetaval1','dist_enc2_nr','dist_enc2_fr','p','pdash','xm','ym_enc2','ym_act','dist_enc2','dist_enc_act','dist_dec_act','erm_enc2','erm_act','drm_act','xrm','xrandinit','exitflagrn','sigma_thsq','sigma_xsq','dist_enc_nr','dist_dec_nr','dist_enc_fr','dist_dec_fr');
end
...
dist_enc=dist_enc+pval(encind)*integral(f5,xthetam(j,i),xthetam(j,i+1),'ArrayValued',true);
end
end
